...
sed -i "s|export let ip = \"http://localhost:5000\";|export let ip = \"http://$SERVER_IP:5000\";|g" "$file_path"
...
if grep -q "export let ip = \"http://$SERVER_IP:5000\";" "$file_path"; then
  echo "IP address successfully updated to http://$SERVER_IP:5000 in $file_path"
else
